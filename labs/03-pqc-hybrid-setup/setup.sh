...
if [ -f "certs-hybrid/p256_mldsa65_cert.pem" ] && [ -f "certs-hybrid/p256_mldsa65_key.pem" ]; then
    echo "✓ Hybrid certificates found"
else
    echo "⚠ Certificates not found"
    echo ""
    echo "You need to generate PQC hybrid certificates."
    echo ""
    echo "Options:"
    echo "  1. Use pre-compiled OpenSSL+OQS binaries (RECOMMENDED)"
    echo "     - Extract to /usr/local/"
    echo "     - Run: ./scripts/generate-hybrid-cert.sh"
    echo ""
    echo "  2. Compile OpenSSL+OQS from source"
    echo "     - See: guides/install-oqs.md"
    echo "     - Then run: ./scripts/generate-hybrid-cert.sh"
    echo ""
    read -p "Do you want to continue without certificates? [y/N] " -n 1 -r
    echo
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
        echo "Setup cancelled. Generate certificates first."
        exit 1
    fi
fi
echo ""
